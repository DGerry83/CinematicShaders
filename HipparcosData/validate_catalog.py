...
def read_star(f, index):
    ...
    dx, dy, dz, mag, r, g, b, temp = struct.unpack('<ffffffff', f.read(32))
    return {
        'dir': (dx, dy, dz),
        'mag': mag,
        'color': (r, g, b),
    ...
    with open(filepath, 'rb') as f:
        ...
        if header['star_count'] > 0:
            ...
            for i in [0, min(4, header['star_count']-1), header['star_count']//2, header['star_count']-1]:
                ...
                dx, dy, dz = star['dir']
                r, g, b = star['color']
                print(f"  [{i:6d}] dir=({dx:7.4f},{dy:7.4f},{dz:7.4f}) mag={star['mag']:6.2f} "
                      f"rgb=({r:.3f},{g:.3f},{b:.3f}) T={star['temp']:6.0f}K")
